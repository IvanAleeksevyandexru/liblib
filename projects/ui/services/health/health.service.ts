...
export class HealthService {
  ...
  private send(event, time, result, eventInfo?, customPageId= ''): void {
    ...
    let pageId = customPageId ? customPageId :
      (window.location.pathname + window.location.hash).replace(/[(#\/):\.&\?=]/g, '_').slice(1).toLowerCase();
  ...
  public measureEnd(id, result, eventInfo, customPageId= ''): void {
    const startTime = this.measures[id];
    if (startTime) {
      const delta = Date.now() - startTime;
      this.send(id, delta, result, eventInfo, customPageId);
