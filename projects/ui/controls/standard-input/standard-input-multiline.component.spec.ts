...
@Component({
  template: `<lib-standard-input #input [multiline]="true" [(ngModel)]="value" [commitOnInput]="false"></lib-standard-input>`
})
...
@Component({
  template: `<div [formGroup]="form"><lib-standard-input formControlName="control"
      [multiline]="true" [commitOnInput]="false"></lib-standard-input></div>`
})
...
@Component({
  template: `<lib-standard-input #managed (focus)="otherInputFocusReceived()"
    ...
    <lib-standard-input #input (focus)="focusReceived()" (blur)="inputBlured()"
              contextClass="base" [multiline]="true"></lib-standard-input>
...
@Component({
  template: `<lib-standard-input [multiline]="true" (change)="change()"
...
describe('Multiline StandardInputComponent', () => {
  ...
  it('should bind-out when used with ngModel', () => {
    ...
    expect(ngModel.value).toBe('new user \nvalue');
  });
  ...
  it('should emit focus, blur, input and change events as usual textarea', fakeAsync(() => {
    const hostFixture = TestBed.createComponent(EventTestComponent);
    hostFixture.detectChanges();
    tick();
    input = hostFixture.nativeElement.querySelector('.text-input') as HTMLInputElement;
