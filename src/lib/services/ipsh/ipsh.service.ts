...
import {
  Bill, BillAttr, BillDiscounts,
  BillResponse, BillsErrors,
  BillsRequestParams,
  ErrorInfo,
  PayOption
...
import {
  ...
  GibddPhotoResponse
} from '../../models/gibdd-fine.model';
import { PaymentDetailsContent, PaymentDetailsResponse, PaymentErrors, StatusComment } from '../../models/payment.model';
...
export class IpshService {
  ...
  public getPaymentStatusComment(params): Observable<StatusComment> {
    const {status, createTime, fKQuittanceStatus, fKQuittanceComment, ipPaymentAmount, category,
      statusUpdated, billNumber, multiplierSize} = params;
    let comment = '';
    let param = '';
