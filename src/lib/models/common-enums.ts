...
export enum SearchSyncControl {
  ...
  PARALLEL = 'parallel', // синхронизация отсутствует, исходящий поиск может запускаться когда поиск в процессе (аналог Subject)
  QUEUE = 'queue', // использовать механизм очереди на уровне компонента (аналог ReplaySubject)
  LAST_STANDING = 'last-standing' // помнить только один последний отложенный элемент, не проигрывая всю очередь (аналог BehaviorSubject)
}
